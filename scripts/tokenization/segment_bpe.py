...
Train a SentencePiece model and apply it to the Mapudungun side of the corpus.
Produces three conditions:

  --mode duan    Joint arn+es BPE with 5,000 vocab size, replicating Duan et al. (2020).
                 Output: data-processed/{approach}/joint_bpe/{split}/{src,tgt}.txt
  ...
  --mode large   Language-specific arn-only BPE. Vocabulary size determined by the
                 ...
                 Output: data-processed/{approach}/mono_bpe/{split}/{src,tgt}.txt

  --mode unigram Language-specific arn-only SentencePiece UnigramLM. Uses the same
                 Gowda & May (2020) 95%-coverage heuristic for vocabulary size.
                 Output: data-processed/{approach}/unigram_lm/{split}/{src,tgt}.txt
...
Usage:
    ...
    uv run python scripts/tokenization/segment_bpe.py --mode unigram
...
def parse_args():
    ...
    parser.add_argument("--mode", choices=["duan", "large", "unigram"], required=True,
                        help="duan: 5K joint BPE (Duan et al. 2020); "
                             "large: arn-only BPE with 95%%-coverage vocab; "
                             "unigram: arn-only SentencePiece UnigramLM with 95%%-coverage vocab")
    ...
    parser.add_argument("--out-dir-name", default=None,
                        help="Override output directory name (default: joint_bpe or mono_bpe)")
...
def train_sp_model(input_files: list[Path], model_prefix: Path,
                   vocab_size: int, model_type: str = "bpe",
                   character_coverage: float = 1.0):
    """Train a SentencePiece model (BPE or Unigram)."""
...
def main():
    ...
    for approach in args.approaches:
        ...
        # ------------------------------------------------------------------
        if args.mode == "duan":
            model_prefix = bpe_root / f"{approach}_joint_bpe"
            ...
            out_dir_name = args.out_dir_name or "mono_bpe"

        else:  # unigram
            vocab_size = args.large_vocab
            if vocab_size is None:
                vocab_size = compute_95_coverage_vocab_size(arn_train)
            model_prefix = bpe_root / f"{approach}_unigram_lm_{vocab_size}"
            print(f"Training arn-only UnigramLM (vocab_size={vocab_size})...")
            train_sp_model(
                input_files=[arn_train],
                model_prefix=model_prefix,
                vocab_size=vocab_size,
            ...
            out_dir_name = args.out_dir_name or "unigram_lm"
