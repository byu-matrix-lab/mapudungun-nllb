...
Output uses the same @@ convention as segment_morfessor.py, so:
  ...
  - TOKENIZER_APPROACH=cascade feeds into the same desegmentation pipeline

Input: blocks/morfessor/{split}/src.txt  (Morfessor-segmented arn)
Output: blocks/cascade/{split}/src.txt   (cascade-merged arn, fewer @@ markers)
        blocks/cascade/{split}/tgt.txt   (unchanged es, copied from cleaned)
...
# ---------------------------------------------------------------------------
# NLLB-constrained cascade merge
# ---------------------------------------------------------------------------
...
def cascade_line(line: str, vocab: set[str]) -> str:
    """Apply NLLB-constrained cascade merging to one Morfessor-segmented line."""
...
def main():
    ...
    for approach in args.approaches:
        ...
        cascade_base   = data_root / approach / "cascade"
